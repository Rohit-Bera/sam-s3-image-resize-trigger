...
```bash
sam-s3-img$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam-s3-img$ sam local start-api
sam-s3-img$ curl http://localhost:3000/
```
...
```bash
sam-s3-img$ sam logs -n HelloWorldFunction --stack-name sam-s3-img --tail
```
...
```bash
sam delete --stack-name sam-s3-img
```
    ...
    "Statement": [
        {
            ...
            "Principal": "*",
            "Action": "s3:GetObject",
            "Resource": "arn:aws:s3:::nodejs-learning/*"
        }
    ]
}
